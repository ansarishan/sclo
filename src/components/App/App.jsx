...
const App = () => {
  ...
  const addBoxes = (e) => {
    ...
    if (last > 0) {
      ...
      if (SameSizeBox[last - 1].position[1] > 4) {
       
        x1 = SameSizeBox[last - 1].position[0] + values.boxWidth;
        y1 =SameSizeBox[last - 1].position[1] + values.boxHeight / 2;
        z1 = SameSizeBox[last - 1].position[2];
      // Check if already box present
      var bottomBox = values.boxes.filter(b=> b.position[2]==z1 )
      if(bottomBox.length>0){

       let box = _.max(bottomBox, function(o){return o.position[0];})
       
        x1 = box.position[0]+ box.size[0]/2 + values.boxWidth/2;
        y1 =values.boxHeight / 2;
        if(x1>10){


          // x1 =-10 + values.boxWidth / 2;
          // y1 = values.boxHeight / 2;
          // z1 =3;
        }
      ...
      setValues({
        ...values, boxes: [...values.boxes, { position: [x1, y1, z1], size: [values.boxWidth, values.boxHeight, values.boxLength] }],
      ...
      // If same length
      let SameLengthBox = values.boxes.filter(f => f.size[2] === values.boxLength);
      let box = _.max(SameLengthBox, function(o){return o.position[0];})
      let LatSameLengthBox = SameLengthBox.length;
      if (LatSameLengthBox > 0) {
        x1 = box.position[0] + box.size[0] / 2 + values.boxWidth / 2;
       console.log(SameLengthBox)
       console.log(box)
        console.log('gg '+x1)
